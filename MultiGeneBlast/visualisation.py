...
"""
Visualisation functions for the creation of SVG's and writing of  the html page
...
from utilities import is_valid_accession, MultiGeneBlastException
...
class ClusterCollectionSvg:
    ...
    def __calculate_distance_per_nucleotide(self, all_clusters):
        ...
        for cluster in all_clusters:
            start, stop_ = cluster.core_start_stop()
            size = (stop_ - start) + 2 * SVG_CORE_EXTENSION
    ...
    def __configure_cluster_svgs(self, clusters, query_cluster, gene_color_dict, user_options):
        ...
        if user_options.architecture_mode:
            line_ = False
        cluster_svgs["query"] = ClusterSvg(list(query_cluster.proteins.values()), self.dpn, 0, (self.width * 0.75),
                                           gene_color_dict, line_=line_)
...
class ClusterSvg:
    ...
    """
    def __init__(self, proteins, dpn, index, screenwidth, color_dict, line_=True, reverse=False):
        """
        ...
        colors so all proteins with the same blast subject have the same color
        :param line_: a boolean that tells if a lign should be drawn trough all the
        gene shapes
        ...
        self.cluster_image_groups = self._cluster_image(line_)
    ...
    def __configure_gene_arrows(self, proteins, start_offset, color_dict):
        ...
        for prot in proteins:
            if prot.name in color_dict:
                ...
                color = "#FFFFFF"
            arrows[prot.name] = GeneArrow(prot, start_offset, self.start, self.dpn, color, self.start_height,
                                          reverse=self.reversed)
        return arrows

    def _cluster_image(self, line_):
        """
        ...
        group = g()
        if line_:
            group.addElement(builder.createLine(10, self.start_height - 5, self.total_width, self.start_height - 5,
...
class GeneArrow:
    ...
    def _gene_arrow(self, start_height, height):
        ...
        pg = builder.createPolygon(points=builder.convertTupleArrayToPoints(points_as_tuples), strokewidth=1,
...
def create_xhtml_template(html_parts, page_indx, page_sizes):
    ...
    inner_string = "var list=[{},'all'];"\
        .format(",".join([str(nr + page_indx * 50 + 1) for nr in range(page_sizes[page_indx])]))
...
def write_xhtml_output(html_outfile, clusters, query_cluster, page_indx, user_options, svg_images, gene_color_dict):
    """
    ...
    # Add menu bar 3
    html_outfile.write('<div id="bartext3" style="color:#FFFFFF; font-size:1em; position:absolute; z-index:2; top:3px;'
                       ' left:20px;"><b>MultiGeneBlast hits</b></div>')
    html_outfile.write('<div id="descrbar3" style="position:absolute; z-index:1; top:0px;"><img src="images/bar.png"'
                       ' height="25" width="{}"/></div>'.format(int(screenwidth * 0.75)))
    html_outfile.write('<div class="help" id="help3" style="position:absolute; z-index:1; top:2px; left:{}px;"><a'
                       ' href="http://multigeneblast.sourceforge.net/usage.html" target="_blank"><img border="0"'
                       ' src="images/help.png"/></a></div>'.format(int(screenwidth * 0.75 - 30)))
    page_nr = page_indx + 1
    html_outfile.write('<div id="qcluster{}">\n<br/><br/>\n<div align="left">\n<form name="clusterform{}">\n<select'
                       ' name="selection{}" onchange="javascript:navigate(this);">\n'.format(page_nr, page_nr, page_nr))
    ...
    html_outfile.write('<div style="position:absolute; top:33px; left:{}px;"><img src="images/button.gif"'
                       ' name="button{}" onclick="javascript:displaybutton({});"/></div>'
                       .format(screenwidth * 0.625, page_nr, page_nr))
    for index, cluster in enumerate(clusters):
        ...
        html_outfile.write('\n{}id="svg{}_{}" >\n'.format(svg_lines[0][:-1], page_nr, index + 1 + page_indx
                                                          * HITS_PER_PAGE))
        name_index = -1
        protein_index = 0
        ...
        if is_valid_accession(cluster.contig):
            cluster_desc = 'Cluster{}: <a href="http://www.ncbi.nlm.nih.gov/nuccore/{}" target="_blank"> {}</a>' \
                           ' {}&nbsp;&nbsp;&nbsp;&nbsp;Total score: {:.1f}&nbsp;&nbsp;&nbsp;&nbsp; Cumulative Blast' \
                           ' bit score {:.2f}'.format(cluster.no, cluster.contig, cluster.contig, contig_desc,
                                                      cluster.score,
                                                      cluster.segmented_score("accumulated blast score")[0] * 1_000_000)
        else:
            cluster_desc = 'Cluster{}: {} {}&nbsp;&nbsp;&nbsp;&nbsp;Total score: {:.1f}&nbsp;&nbsp;&nbsp;&nbsp;' \
                           ' Cumulative Blast bit score: {:.2f}'\
                .format(cluster.no, cluster.contig, contig_desc, cluster.scorecluster,
                        cluster.segmented_score("accumulated blast score")[0] * 1_000_000)
        ...
        # Insert NCBI links
        html_outfile.write('<div id="pub_pics" style="position:absolute; top:175px; left:0px;'
                           ' font-size:10px"> Hit cluster cross-links: \n')
        ...
        for pindex, protein in enumerate(query_cluster.proteins.values()):
            html_outfile.write('<div id="q{}_{}_{}_div" class="hidden popup" style="position:absolute; top:100px; '
                               'left:{}px;">\n'.format(page_nr, page_indx * HITS_PER_PAGE + index + 1,
                                                       pindex, int(query_starts[pindex] * 0.875)))
            html_outfile.write("{}\n".format(protein.annotation.replace("_", " ")))
            if protein.protein_id != "" and is_valid_accession(protein.protein_id):
                html_outfile.write('<br/>Accession: <a href="http://www.ncbi.nlm.nih.gov/protein/{}" target="_'
            ...
            html_outfile.write("</div>\n\n")
            html_outfile.write('<div id="q{}_{}_{}_divtext" class="hidden genenames" style="position:absolute;'
                               ' top:75px; left:{}px;">\n'.format(page_nr, page_indx * HITS_PER_PAGE + index + 1,
                                                                  pindex, ((query_starts[pindex] + query_ends[pindex])
                                                                           / 2) * 0.9375))
        ...
        for pindex, protein_name in enumerate(cluster_protein_names):
            ...
            html_outfile.write('<div id="h{}_{}_{}_div" class="hidden popup" style="position:absolute; top:151px;'
                               ' left:{}px;">\n'.format(page_nr, page_indx * HITS_PER_PAGE + index + 1,
                                                        pindex, int(prot_starts[pindex] * 0.875)))
            html_outfile.write("{}\n".format(protein.annotation.replace("_", " ")))
            ...
            if protein_name in cluster.blast_hit_proteins:
                blast_list = sorted(list(cluster.blast_hit_proteins[protein_name]),
                                    key=lambda x: (x.evalue, -x.bit_score))
            ...
            if protein.name in gene_color_dict:
                ...
                path_ = "{}{}muscle_info{}group{}_muscle.fasta".format(user_options.outdir, os.sep, os.sep, color)
                if os.path.exists(path_):
                    html_outfile.write("<br/><a href=\"{}\" target=\"_blank\"> Muscle alignment of this gene with"
                                       " homologs </a>\n".format(path_))
            html_outfile.write("</div>\n\n")
            html_outfile.write('<div id="h{}_{}_{}_divtext" class="hidden genenames" style="position:absolute;'
                               ' top:126px; left:'
                               '{}px;">\n'.format(page_nr, page_indx * HITS_PER_PAGE + index + 1, pindex,
                                                  ((prot_starts[pindex] + prot_ends[pindex]) / 2) * 0.9375))
            if protein.locus_tag != "":
                html_outfile.write(protein.locus_tag)
            else:
                html_outfile.write(protein.protein_id)
    ...
    for svg_line in all_svg_lines[1:]:
        if svg_line.startswith("</g>"):
            ...
            if names[name_index] == "query":
                ...
                html_outfile.write('<g id="all_{}_{}_{}"  >\n'.format(page_nr, page_indx * HITS_PER_PAGE + name_index,
                                                                      protein_index))
            html_outfile.write(svg_line + "\n")
            html_outfile.write('</g>\n')
            protein_index += 1
    ...
    if len(query_desc) < 90:
        ...
        query_desc = "Query: {}...".format(query_desc[:87])
    html_outfile.write('<div id="descriptionquery" style="text-align:left; position:absolute; top:60px; left:10px; '
                       'font-size:10px; font-style:italic">{}</div>\n'.format(query_desc))
    ...
    for index, cluster in enumerate(clusters):
        ...
        if is_valid_accession(cluster.contig):
            ...
            cluster_desc = 'Cluster{}: {} {}&nbsp;&nbsp;&nbsp;&nbsp;Total score: {:.1f}&nbsp;&nbsp;&nbsp;&nbsp;' \
                ...
                .format(cluster.no, cluster.contig, contig_desc, cluster.scorecluster,
                        cluster.segmented_score("accumulated blast score")[0] * 1_000_000)
        html_outfile.write('<div id="description{}" style="text-align:left; position:absolute; top:{}px; left:10px; '
                           'font-size:10px; font-style:italic">{}</div>\n'.format(page_nr,
                                                                                  int(63 + (51.7 * (index + 1))),
                                                                                  cluster_desc))
    ...
    for index, cluster in enumerate(clusters):
        if index == 0:
            ...
            for pindex, protein in enumerate(query_cluster.proteins.values()):
                html_outfile.write('<div id="all_{}_0_{}_div" class="hidden popup" style="position:absolute; top:100px;'
                ...
                html_outfile.write("<br/>Size: {} bp\n".format(int(protein.stop - protein.start)))
                html_outfile.write("</div>\n\n")
                html_outfile.write('<div id="all_{}_0_{}_divtext" class="hidden genenames" style="position:absolute;'
                                   ' top:75px; left:''{}px;">\n'
        ...
        for pindex, protein_name in enumerate(cluster_protein_names):
            protein = cluster.get_protein(protein_name)
            html_outfile.write('<div id="all_{}_{}_{}_div" class="hidden popup" style="position:absolute; top:{}px;'
                               ' left:{}px;">\n'.format(page_nr, index + 1 + page_indx * HITS_PER_PAGE,
                                                        pindex, int(100 + (51.7 * (index + 1))),
                                                        int(prot_starts[pindex] * 0.875)))
            html_outfile.write("{}\n".format(protein.annotation.replace("_", " ")))
            ...
            if protein.name in gene_color_dict:
                ...
                if os.path.exists(path_):
                    html_outfile.write("<br/><a href=\"{}\" target=\"_blank\"> Muscle alignment of this gene with "
                                       "homologs </a>\n".format(path_))
            html_outfile.write("</div>\n\n")
            html_outfile.write('<div id="all_{}_{}_{}_divtext" class="hidden genenames" style="position:absolute; '
                               'top:{}px; left:{}px;">\n'.format(page_nr, index + 1 + page_indx * HITS_PER_PAGE,
                                                                 pindex, int(75 + (51.7 * (index + 1))),
                                                                 ((prot_starts[pindex]+prot_ends[pindex])/2)*0.9375))
            if protein.locus_tag != "":
                html_outfile.write(protein.locus_tag)
            else:
    ...
    html_outfile.write('</div>\n\n')
    html_outfile.write('</div>\n')
    html_outfile.write('<div id="creditsbar{}" class="banner" style="position:absolute; width:{}px; align:\'left\';'
                       ' height:75; top:2750px; left:0px; color:#000066; z-index:-1;">'
                       .format(index, int(0.98 * screenwidth)))
    html_outfile.write('<div style="float:center; font-size:0.9em;">\n<div style="position:absolute; top:0px; '
                       'left:30px;">\n<img src="images/ruglogo.gif" border="0"/>&nbsp;&nbsp;&nbsp;&nbsp;\n<img '
                       'src="images/gbblogo.gif" border="0"/>&nbsp;&nbsp;&nbsp;&nbsp;\n</div>\n<div style="position'
                       ':absolute; top:10px; left:340px;">\nDetecting sequence homology at the gene cluster level '
...
def create_xhtml_file(clusters, query_cluster, user_options, svg_images, page_sizes, page_indx, gene_color_dict):
    ...
    write_xhtml_output(html_outfile, clusters, query_cluster, page_indx, user_options, svg_images, gene_color_dict)
